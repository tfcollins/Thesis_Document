...
\centering
\includegraphics[scale=0.2]{usrp2_overview.eps}
\caption{Full USRP2 Hardware with Daughtercard}
\end{figure}

\begin{figure}\label{usrp2_mainboard}
\centering
\includegraphics[scale=0.2]{usrp2_mainboard.eps}
...
\centering
\includegraphics[scale=0.4]{SS_Equalizer_sim.eps}
...
\begin{enumerate}
\item The data sequence \(x_{d}\) is an i.i.d. sequence such that \(x_{d}~\mathcal{CN}(0,\sigma_{d}^{2})\)

\item The distribution over the MIMO channel vector is \(p(y;\theta)~\mathcal{CN}(\mu_{y},R_{w})\), and the interference vector is distrbuted normal with covariance \(R_{w}\)
