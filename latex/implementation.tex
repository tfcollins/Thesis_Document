...
\begin{figure}\label{bliss_system}
\includegraphics{bliss_system.eps}
\end
...
\begin{figure}
\includegraphics[scale=0.75]{usrp2_overview.eps}
\includegraphics[scale=0.75]{usrp2_mainboard.eps}
\end
...
\begin{figure}
\includegraphics[scale=0.75]{sdr_block_diagram}
\cite{sdr_blocks}
\end
...
\includegraphics{mimo.eps}%taken on iphone
\end

\begin{figure}\label{mimo_grc}
\includegraphics{mimo_grc.eps}
\end
...
\includegraphic{SS_Equalizer.eps}
\end
...
\includegraphic{SS_Equalizer_small.eps}
\end
...
\includegraphics{ss_conv_ber.eps}
\end
...
\begin{figure}
%changing power graph of subtraction
\includegraphics{ss_conv_changing_over_subtraction_level.eps}
\end
...
\includegraphics{ss_conv_phase_offsets.eps}
\end

As you can see from figures \ref{} and \ref{}, the subtraction mechanism is extremely sensitive to timing offsets, while frequency shifts the system is rather robust.  If a full bitshift occures though, the entire system falls apart.  This is a rather large problem and must be considered when attempting such operation on non-simulated data.  This problem was forsean because of the time variance or sensitive of the signals themselves. \\
...
\begin{figure}
\includegraphic{code_work_flow.eps}
\end
...
\includegraphics{grc_receiver.eps}
\end
...
\begin{figure}
%Combined signals \includegraphics{combo_signal_time.eps}
\end
...
With our channel estimation method chosen, a simulation was created to prove the effectiveness of such a scheme.  The result of this implementation were directly compared with the results of the paper to prove correctness, and compared with traditional equalizer performance.  The results of this simulation are seen below.  It examines a random frequency selective channel, with very high suppression with a across a number of SNR values.  As expected there is a linear relationship between SNR.  For completeness a comparision was done with a traditional LMS equalizer to show the effectiveness of the superimpose equalizer.\\
...
\includegraphics{{ants_board.eps}
\end
